...
async function assembleGptProDraft() {
  ...
  // 스토리텔링 배경 설명 지침
  draft += `[배경 설명 스토리텔링 지침]\n`;
  draft += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  draft += `역사적/신학적 배경을 설명할 때 딱딱한 정보전달을 금지합니다.\n\n`;
  draft += `✗ 나쁜 예:\n`;
  draft += `  "이사야서는 주전 8세기에 기록되었으며, 당시 앗수르의 위협이..."\n\n`;
  draft += `✓ 좋은 예:\n`;
  draft += `  "지금 이 순간, 이스라엘 백성들은 두려움에 떨고 있습니다.\n`;
  draft += `   앗수르라는 거대한 제국이 코앞까지 밀려왔기 때문입니다.\n`;
  draft += `   모든 이웃 나라들이 하나씩 무너지는 것을 보면서,\n`;
  draft += `   '우리는 살아남을 수 있을까?' 이것이 당시 모든 사람들의 질문이었습니다."\n\n`;
  draft += `- 역사적 배경을 '사실 나열'이 아닌 '장면 묘사'로 전달하세요\n`;
  draft += `- 청중이 그 시대, 그 장소에 있는 것처럼 느끼게 하세요\n`;
  draft += `- 인물의 감정, 고민, 상황을 생생하게 그려주세요\n`;
  draft += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;

  // 대화체 문체 지침 (★ 매우 중요)
  draft += `[문체 지침 - 대화하듯 친근하게] ★★★\n`;
  draft += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  draft += `설교는 '글'이 아니라 '말'입니다. 강단에서 청중을 바라보며 말하는 것처럼 쓰세요.\n\n`;
  draft += `✗ 딱딱한 문체 (금지):\n`;
  draft += `  "믿음의 첫 걸음은 떠남입니다. 떠남은 로맨틱한 모험이 아니라,\n`;
  draft += `   익숙함을 내려놓는 과감한 순종입니다."\n\n`;
  draft += `✓ 대화체 문체 (권장):\n`;
  draft += `  "믿음의 첫 걸음이 뭘까요? 떠남이에요.\n`;
  draft += `   근데 이 떠남, 영화에서 보는 것처럼 멋진 모험이 아니거든요.\n`;
  draft += `   진짜 어렵습니다. 익숙한 것들을 내려놓아야 하니까요."\n\n`;
  draft += `- "~거든요", "~잖아요", "~죠", "~네요" 같은 부드러운 어미 사용\n`;
  draft += `- 청중에게 질문을 던지며 함께 생각하게 하세요\n`;
  draft += `- 정의(定義)보다 예시와 이야기로 설명하세요\n`;
  draft += `- "여러분", "우리" 등 청중을 직접 부르는 표현 자주 사용\n`;
  draft += `- 너무 격식적인 한자어보다 쉬운 우리말 선택\n`;
  draft += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;

  // 대지/소대지 지침
  draft += `[대지/소대지 작성 지침] ★★★\n`;
  draft += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  draft += `소대지의 목적: 설교에서 방향을 잃지 않고 흐름을 이어가기 위함\n`;
  draft += `→ 본문 해설이 핵심! 예화/적용을 넣는 곳이 아님\n\n`;
  draft += `✗ 잘못된 구성 (금지):\n`;
  draft += `  "1-1에서 예화, 1-2에서 적용, 2-1에서 예화..." → 예화/적용 폭탄\n\n`;
  draft += `✓ 올바른 구성 (권장):\n`;
  draft += `  "1-1 본문 해설 → 1-2 본문 해설 → 2-1 본문 해설 + 예화 → ..."\n`;
  draft += `  → 본문 해설이 중심, 예화는 전체에서 1-2개만\n\n`;
  draft += `★ 예화/적용 배치 원칙:\n`;
  draft += `  - 예화: 설교 전체에서 1-2개만 (대지마다 넣지 말 것!)\n`;
  draft += `  - 적용: 결론에 집중 (본론에서는 거의 안 함)\n`;
  draft += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
