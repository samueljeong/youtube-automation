...
(function() {
  ...
  var TubeLens = {
    ...
    getTranscript: function(videoId, title) {
      var self = this;
      var btn = document.getElementById('transcript-btn-' + videoId);
      ...
      fetch('/api/tubelens/transcript', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          videoId: videoId,
          languages: ['ko', 'ja', 'en', 'zh-Hans', 'zh-Hant'],
          includeAuto: true
        })
      })
      .then(function(res) { return res.json(); })
      .then(function(data) {
        ...
        if (data.success) {
          self.currentTranscript = data.data;
          self.currentTranscriptTitle = title || videoId;
          self.showTranscriptModal();
        } else {
          alert(data.message || '대본을 추출할 수 없습니다.');
        }
      })
      .catch(function(error) {
        if (btn) {
        ...
        console.error('[TubeLens] Transcript error:', error);
        alert('대본 추출 중 오류가 발생했습니다.');
      });
    },

    showTranscriptModal: function() {
      if (!this.currentTranscript) return;

      var modal = document.getElementById('transcript-modal');
      var titleEl = document.getElementById('transcript-modal-title');
      var infoEl = document.getElementById('transcript-modal-info');
      var textEl = document.getElementById('transcript-text');

      var data = this.currentTranscript;

      titleEl.textContent = this.currentTranscriptTitle;

      // Info badges
      var langBadge = '<span>' + (data.language || '알 수 없음') + '</span>';
      var typeBadge = data.isAutoGenerated
        ? '<span class="auto">자동생성</span>'
        : '<span class="manual">수동</span>';
      var charBadge = '<span>' + data.charCount.toLocaleString() + '자</span>';
      var timeBadge = '<span>약 ' + data.estimatedMinutes + '분</span>';

      infoEl.innerHTML = langBadge + typeBadge + charBadge + timeBadge;

      // Transcript text
      textEl.textContent = data.fullText;

      modal.classList.add('show');
    },

    closeTranscriptModal: function() {
      var modal = document.getElementById('transcript-modal');
      modal.classList.remove('show');
    },

    copyTranscript: function() {
      if (!this.currentTranscript || !this.currentTranscript.fullText) {
        alert('복사할 대본이 없습니다.');
        return;
      }

      var text = this.currentTranscript.fullText;
    ...
    downloadTranscript: function() {
      ...
      var text = this.currentTranscript.fullText;
      var filename = (this.currentTranscriptTitle || 'transcript').replace(/[^a-zA-Z0-9가-힣\s]/g, '_') + '.txt';
    ...
    displayAnalyzerCards: function(videos) {
      ...
      videos.forEach(function(v, idx) {
        ...
        html += '<button class="analyzer-card-btn transcript" id="transcript-btn-' + v.videoId + '" onclick="TubeLens.getTranscript(\'' + v.videoId + '\', \'' + self.escapeHtml(v.title) + '\')">대본</button>';
