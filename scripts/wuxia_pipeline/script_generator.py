...
SCRIPT_TARGET_LENGTH = SCRIPT_CONFIG.get("target_chars", 14000)    # 28분 분량
SCRIPT_MIN_LENGTH = SCRIPT_CONFIG.get("min_chars", 12000)          # 최소 24분
SCRIPT_MAX_LENGTH = SCRIPT_CONFIG.get("max_chars", 16000)          # 최대 32분
...
```json
{{{{
  "script": "[나레이션] 대본 내용... (12,000~16,000자, 약 28분)",
  "chapters": [
    {{{{
      "chapter_number": 1,
...
★★★ 대본 구조 (28분 장편) ★★★
════════════════════════════════════════
5개 챕터로 구성, 각 챕터 약 2,800자 (약 5~6분):
