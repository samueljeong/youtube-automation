...
|------|------|
| ✅ 승인 (APPROVED) | 커밋 진행 |
| ⚠️ 수정 필요 (NEEDS_REVISION) | 피드백 반영 후 재검토 요청 |
| ❌ 거부 (REJECTED) | 재작성 후 재검토 요청 |
...
║  🚨 최우선 경계 대상                                            ║
║                                                                 ║
║  1. 코드 꼬임 (Code Tangling)                                   ║
║     - 함수 A가 함수 B를 호출하는데, B가 다시 A를 참조            ║
║     - 전역 변수 의존성이 꼬여있는 경우                           ║
║     - 수정했더니 다른 곳이 깨지는 구조                           ║
║                                                                 ║
║  2. 코드 중복 (Code Duplication)                                ║
║     - 비슷한 로직이 여러 파일에 흩어져 있음                      ║
║     - 같은 기능을 하는 함수가 이미 존재하는데 새로 만듦          ║
║     - 복붙 코드가 미세하게 다른 버전으로 존재                    ║
╚════════════════════════════════════════════════════════════════╝
```
...
| 항목 | 질문 |
|------|------|
| 꼬임 방지 | 이 코드가 다른 코드에 영향을 주는가? 주면 어디까지? |
| 중복 방지 | 이미 비슷한 함수/로직이 있는가? Grep으로 먼저 검색했는가? |
| 의존성 | 이 함수를 호출하는 곳은? 호출되는 곳은? |
| 부작용 | 전역 상태를 변경하는가? 다른 곳에서 그 상태를 쓰는가? |

**리뷰 프로세스:**
1. **급하게 승인하지 않는다**
2. 수정된 파일뿐 아니라 **관련된 파일**도 함께 본다
3. 기존에 비슷한 코드가 있는지 **Grep으로 먼저 검색**한다
4. 확신이 없으면 **더 조사**한다

### 상세 지침
📄 `docs/CODE_REVIEW_AGENT.md` 참조
