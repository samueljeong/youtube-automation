...
[1대지] (대지 제목)
  ...
  1-1. (소대지 제목)
  - 본문 해설: 해당 구절의 의미를 풀어서 설명
  - 흐름 유지: 대지의 핵심 메시지와 연결

  1-2. (소대지 제목)
  - 본문 해설: 다른 각도에서 같은 흐름을 이어감
  - (필요시) 예화 또는 적용 1개

[2대지] (대지 제목)
(동일한 구조 - 소대지 2개로 흐름 유지)

[3대지] (대지 제목)
(클라이맥스 - 가장 강력한 메시지, 소대지로 절정 구성)

[결론]
(핵심 요약)
(구체적 적용/결단 촉구 - 적용은 여기에 집중!)
(축복/기도)

★ 소대지의 목적:
- 설교에서 방향을 잃지 않기 위함
- 같은 흐름을 계속 이어가기 위함
- 예화/적용을 넣는 곳이 아님 (본문 해설이 핵심!)

★ 예화/적용 배치 원칙:
- 예화: 설교 전체에서 1-2개만 (대지마다 넣지 말 것!)
- 적용: 결론에 집중 (본론에서는 거의 안 함)
...
2. 대지/소대지는 본문 해설에 집중 (★ 매우 중요)
   - 소대지의 목적: 방향을 잃지 않고 흐름을 이어가기 위함
   - 각 소대지에서 본문을 천천히 풀어서 설명
   - 예화/적용은 남발하지 말 것!

   ✗ 잘못된 구성 (금지):
   "1-1에서 예화, 1-2에서 적용, 2-1에서 예화, 2-2에서 적용..."
   → 예화/적용 폭탄, 본문 해설이 사라짐

   ✓ 올바른 구성 (권장):
   "1-1 본문 해설 → 1-2 본문 해설 → 2-1 본문 해설 + 예화 → ..."
   → 본문 해설이 중심, 예화는 전체에서 1-2개만
...
8. 문체는 대화하듯 친근하게 (★ 매우 중요)
   설교는 '글'이 아니라 '말'입니다. 강단에서 청중을 바라보며 말하는 것처럼 쓰세요.
   ...
   ✓ 대화체 문체 (권장):
   "믿음의 첫 걸음이 뭘까요? 떠남이에요.
    근데 이 떠남, 영화에서 보는 것처럼 멋진 모험이 아니거든요.
    진짜 어렵습니다. 익숙한 것들을 내려놓아야 하니까요."

   - "~거든요", "~잖아요", "~죠", "~네요" 같은 부드러운 어미 사용
   - 청중에게 질문을 던지며 함께 생각하게 하라
   - 정의(定義)보다 예시와 이야기로 설명하라
   - "여러분", "우리" 등 청중을 직접 부르는 표현 자주 사용
   - 너무 격식적인 한자어보다 쉬운 우리말 선택
